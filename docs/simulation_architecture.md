...
`sim/twin/message.py` 已提供 `MessageEnvelope` 和 `PolicyDecision`。当前 5G→N6→跨域网关→RS-422→ARINC 429→虚拟 LRU→PX4 链的每一跳都携带语义消息摘要和策略决策；正常场景使用 `allowed`，网关拒绝使用 `denied`，下游阻断使用 `blocked`，事件对外仍保留 `accepted/rejected/blocked` 兼容状态。

当前实现仍是合成模型编排，不是物理协议栈或真实设备控制。下一步是在相同契约下替换各阶段的结果映射为独立协议适配器，并把 `RunManifest` 接入一键编排器。
